...
export function encodeMessageObject(message: MessageObject): Uint8Array {
  const bb = popByteBuffer();
  _encodeMessageObject(message, bb);
  return toUint8Array(bb);
}

function _encodeMessageObject(message: MessageObject, bb: ByteBuffer): void {
  // optional DataType dataType = 1;
  const $dataType = message.dataType;
  if ($dataType !== undefined) {
    writeVarint32(bb, 8);
    writeVarint32(bb, encodeDataType[$dataType]);
  }

  // repeated string listData = 2;
  const array$listData = message.listData;
  if (array$listData !== undefined) {
    for (const value of array$listData) {
      writeVarint32(bb, 18);
  ...
  const $stringDataObject = message.stringDataObject;
...
export function decodeMessageObject(binary: Uint8Array): MessageObject {
  return _decodeMessageObject(wrapByteBuffer(binary));
}

function _decodeMessageObject(bb: ByteBuffer): MessageObject {
  ...
  end_of_message: while (!isAtEnd(bb)) {
    ...
    case 0:
      ...
      // optional DataType dataType = 1;
    case 1: {
      message.dataType = decodeDataType[readVarint32(bb)];
      break;
    }

    // repeated string listData = 2;
    case 2: {
      const values = message.listData || (message.listData = []);
...
export interface StringDataObject {
  from?: string;
  to?: string;
  offer?: string;
}

export function encodeStringDataObject(message: StringDataObject): Uint8Array {
  const bb = popByteBuffer();
  _encodeStringDataObject(message, bb);
  return toUint8Array(bb);
}

function _encodeStringDataObject(message: StringDataObject, bb: ByteBuffer): void {
  ...
  if ($offer !== undefined) {
    writeVarint32(bb, 26);
    writeString(bb, $offer);
  }
}

export function decodeStringDataObject(binary: Uint8Array): StringDataObject {
  return _decodeStringDataObject(wrapByteBuffer(binary));
}

function _decodeStringDataObject(bb: ByteBuffer): StringDataObject {
  const message: StringDataObject = {} as any;
  ...
  end_of_message: while (!isAtEnd(bb)) {
    ...
    // optional string offer = 3;
    case 3: {
      message.offer = readString(bb, readVarint32(bb));
      break;
    }
...
function writeVarint64(bb: ByteBuffer, value: Long): void {
  ...
  const size =
    part2 === 0 ?part1 === 0 ?part0 < 1 << 14 ?part0 < 1 << 7 ? 1 : 2 :part0 < 1 << 21 ? 3 : 4 :part1 < 1 << 14 ?part1 < 1 << 7 ? 5 : 6 :part1 < 1 << 21 ? 7 : 8 :part2 < 1 << 7 ? 9 : 10;
